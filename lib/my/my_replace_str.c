...
char *fill_string(char *prev, char *rep, char *next)
{
	...
	char *new_str = NULL;
	unsigned int new_str_length = my_strlen(prev) + my_strlen(next) + my_strlen(rep);

	if ((new_str = malloc(sizeof(char) * (new_str_length + 1))) == NULL)
		return (NULL);
	for (unsigned int i = 0; prev[i]; i++, index++)
		new_str[index] = prev[i];
	for (unsigned int i = 0; rep[i]; i++, index++)
		new_str[index] = rep[i];
	for (unsigned int i = 0; next[i]; i++, index++)
...
char *replace_patern(char *str, int index, char *rep)
{
	...
	if (new_str == NULL)
		return (NULL);
	for (unsigned int i = 0; i < my_strlen(rep); i++)
...
char *replace_patern_modif(char *str, int index, char *pat, char *rep)
{
	...
	if (!prev || !next)
		return (NULL);
	prev[index] = 0;
	if ((new_str = fill_string(prev, rep, next)) == NULL)
		return (NULL);
	free(prev);
	free(next);
	return (new_str);
